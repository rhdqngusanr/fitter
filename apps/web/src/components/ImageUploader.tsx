...
import { MAX_IMAGE_BYTES, type IMAGE_SOURCE_TYPES } from '@fitter/shared';
 ...
 * 의뢰 사진 업로더.
 *
 * 사진마다 **출처를 따로 받는다.** 의뢰 사진은 고객이 인터넷에서 가져온 남의 저작물일
 * 확률이 높아서, 한 의뢰 안에서도 "내가 찍은 것"과 "어디서 본 것"이 섞인다.
 * 의뢰 단위로 한 번만 물으면 그 구분이 사라지고, 나중에 신고가 들어왔을 때
 * 어느 사진이 문제인지 특정할 수 없다.
 *
...
export interface UploadedImage {
  ...
  name: string;
  previewUrl: string;
  sourceType: (typeof IMAGE_SOURCE_TYPES)[number];
  sourceUrl: string;
}
...
export function ImageUploader({ requestId, images, max, onChange, authFetch }: Props) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        added.push({
          ...
          /* 기본값은 "직접 촬영"이 아니다. 잘못 신고되는 쪽이 훨씬 비싸다. */
          sourceType: 'EXTERNAL',
          sourceUrl: '',
        });
      }
  ...
  return (
    <div style={{ marginBottom: 'var(--space-6)' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {img.sourceType === 'EXTERNAL' && (
              <input
                ...
                style={{ ...inputStyle, height: 40, marginTop: 'var(--space-2)', fontSize: 14 }}
              />
            )}
          </div>
        </div>
      ...
      <p style={{ fontSize: 13, color: 'var(--color-text-tertiary)', marginTop: 'var(--space-3)' }}>
        남의 사진을 올릴 때는 원본 주소를 함께 남겨주세요. 권리자가 내려달라고 하면 그 사진만
        내립니다.
      </p>

      {/* 사진 ID를 부모가 알 필요는 없다. 부모는 requestId 만 안다. */}
