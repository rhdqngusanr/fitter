...
**`pnpm qc`는 이 프로젝트 전용 검사다.** 확장 규약 위반(자유 텍스트 평수·지역), 정본 분열(공종 목록·용량 상수), 금지어, 볼트 링크 깨짐, **없는 화면으로 가는 링크**를 잡는다. `design/`은 warn(이미 아는 부채), 코드는 error다. 자세한 건 `brain/90-규약/자동 QC.md`.

### 화면 QC 는 `pnpm qc` 가 아니다

**`pnpm qc` 통과를 "화면을 검사했다"로 읽지 마라.** 이 프로젝트가 실제로 겪은 문제가 그것이다 — 시안과 다른 화면이 전부 통과한 채로 쌓였다. `pnpm test` 도 브라우저를 열지 않는다(도메인 단위 + API e2e).

| 층 | 무엇으로 | 자동인가 |
| --- | --- | --- |
| 1. 정적 | `pnpm qc` | **자동.** Stop 훅도 돈다 |
| 2. 렌더 | **`/qc-screens`** | 불러야 돈다 |
| 3. 판단 | **`/qc-screens`** | 불러야 돈다 |

**화면을 만들거나 고쳤으면 `/qc-screens` 를 돌린다.** 무엇을 왜 보는지는 `brain/90-규약/화면 QC 목록.md` 에 있고, 항목은 전부 실제로 겪은 결함에서 왔다. 화면이 여러 개면 `frontend-qc` 서브에이전트에게 하나씩 맡긴다.
